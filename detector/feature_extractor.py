...
def feature_csv(label, threshold, feature_vec, Arch):
    ...
    column_names = ['filename', 'label', 'arch', 'nodes', 'edges', 'out_degree', 'in_degree', 'density', 'closeness_centrality', 'betweeness_centrality', 'connected_components', 
                    'diameter', 'radius', 'total_trans', 'total_cal', 'total_ctl', 'avg_trans', 'avg_cal', 'avg_ctl', 'avg_block', 'avg_block_size']
    
    # Check if the file exists and is empty
    file_exists = os.path.isfile('./csv_files/train_data.csv')
    
    with open('./csv_files/train_data.csv', 'a+', newline='') as csvfile:
        writer = csv.writer(csvfile)
        
        # Write the header only if the file is new or empty
        if not file_exists or os.stat('./csv_files/train_data.csv').st_size == 0:
